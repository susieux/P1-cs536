...
//	The SymTable class must be in a file named SymTable.java. It must be implemented using a List of HashMaps. 
//	(Think about the operations that will be done on a SymTable to decide whether to use an ArrayList or a LinkedList.) 
//	The HashMaps must map a String to a Sym. This means that the SymTable class will have a (private) field of type List<HashMap<String,Sym>>.
	...
	//If this SymTable's list is empty, throw an EmptySymTableException. If either name or sym (or both) is null, throw a NullPointerException. 
